...
(function (root, factory) {
	...
	const DEFAULTS = {
		...
		liabilityValue: 4.0,
		interactionCap: 45,  // ceiling on board-interaction value for any one card
		prereqPenalty: 1.5,
	};
	...
	function deckContext(deckCards, opts) {
		...
		// Printed statlines only — used to price cards that act on other cards, without
		// recursing back into valueCard.
		const lines = deckCards
			.filter((c) => (c.Type || "Member") === "Member")
			.map((c) => Number(c.FamePerTurn || 0) * o.famePoint + Number(c.MoneyPerTurn || 0) * o.moneyPoint)
			.sort((a, b) => b - a);
		const topStatline = lines.length ? lines[0] : 2;
		const goodStatline = lines.length ? lines[Math.min(lines.length - 1, 4)] : 1;
	...
	/**
	 * Cards whose power is what they do to *other* cards — buffs, copies, denial, repeatable
	 * abilities — carry none of it in a per-turn clause, so the clause parser prices them near
	 * zero. Each rule below values the effect from the deck's own contents and the tempo model:
	 * a turn is worth FAME_GOAL / GAME_LENGTH fame, and an effect earns for the turns left
	 * after its own tier comes online.
	 */
	const TURN_WORTH = FAME_GOAL / GAME_LENGTH;

	const INTERACTIONS = [
		{
			name: "doubles a member's stats",
			re: /double (its|their) stats/i,
			// Adds a second copy of your best board card's output for the rest of the game.
			value: (ctx, m, life) => ctx.topStatline * life,
		},
		{
			name: "buffs a member",
			re: /\+(\d+)\/\+(\d+)/,
			value: (ctx, m, life) =>
				(Number(m[1]) * ctx.opts.famePoint + Number(m[2]) * ctx.opts.moneyPoint) * life,
		},
		{
			name: "copies a member",
			re: /copy to your hand|copy of that card/i,
			// You choose what to copy, so you take the best thing on either board.
			value: (ctx, m, life) => ctx.topStatline * life * 0.8,
		},
		{
			name: "locks the opponent out of a tier",
			re: /can't play cards of the selected tier|can't play Events/i,
			// Denying a turn is worth roughly the fame that turn would have produced.
			value: () => TURN_WORTH * 1.5,
		},
		{
			name: "stops the opponent gaining fame",
			re: /can't gain Fame\/turn/i,
			value: (ctx, m, life) => TURN_WORTH * Math.min(3, life),
		},
		{
			name: "removes a rival",
			re: /select a rival to remove|to remove|kidnap/i,
			// Takes their board card off the table for the rest of the game.
			value: (ctx, m, life) => ctx.goodStatline * life * 0.7,
		},
		{
			name: "negates a member",
			re: /negate|Ball Gag/i,
			value: (ctx, m, life) => ctx.goodStatline * life * 0.5,
		},
		{
			name: "cannot be targeted",
			re: /can't be targeted/i,
			// Immune to the removal and negation everyone else has to play around.
			value: (ctx, m, life) => ctx.opts.famePoint * life * 0.5,
		},
		{
			name: "repeatable ability, every turn",
			re: /Active \(once\/turn\)/i,
			// A selection or draw engine that fires every turn for the rest of the game.
			value: (ctx, m, life) => ctx.opts.drawValue * life * 0.5,
		},
		{
			name: "repeatable ability, once per tier",
			re: /Active \(once\/tier\)/i,
			value: (ctx) => ctx.opts.actionValue * 2,
		},
	];

	/** Sum the interaction rules a card's text matches, capped so nothing runs away. */
	function interactionValue(card, text, ctx) {
		...
		for (const rule of INTERACTIONS) {
			...
			if (!m) continue;
			const v = rule.value(ctx, m, life);
			hits.push({ name: rule.name, value: v });
			total += v;
		}
		// A card that comes back to hand runs its effect several times in a 16-turn game.
		if (/returns? to (your |their )?hand|return this card/i.test(text) && total > 0) {
			total *= 1.8;
			hits.push({ name: "returns to hand, so it fires again", value: total / 1.8 * 0.8 });
		}
		const cap = ctx.opts.interactionCap;
		return { total: Math.min(total, cap), hits };
	}
	...
	function valueCard(card, ctx, getText) {
		...
		value += interactionValue(card, text, ctx).total;
	...
	return {
		...
		GAME_LENGTH, TIER_ONLINE, tierLifetime, tierOf, hasAny, parseClauses, parseOneShots,
		deckContext, valueCard, INTERACTIONS, interactionValue,
